...
//
// ✅ DÜZELTİLDİ: 50+ syntax hatası (alignItems: 'c enter', & &, () = > vb.)
// ✅ DÜZELTİLDİ: stale closure riski (setKullanici functional update)
...
const emaildenKey = (email) => email.replace(/\./g, '_').replace(/@/g, '_at_');
...
export function ProfilEkrani({ kullanici, setKullanici, token, rol, setEkran, setSikayetHedef, setSikayetModalAcik, s }) {
  ...
  const belgeYukle = async (tip) => {
    ...
    } finally { setBelgeYukleniyor(false); }
  };
        ...
        {aktifSekme === 'profil' && (
          <>
            ...
            <View style={{ backgroundColor: '#E1F2FE', padding: 15, borderRadius: 12, marginBottom: 15, alignItems: 'center' }}>
              ...
              <Text style={{ color: '#1B4965', fontSize: 20, fontWeight: '900', letterSpacing: 3 }}>{kullanici?.referansKodu || ''}</Text>
            </View>
            ...
            {rol === 'usta' && (
              <>
                ...
                <TextInput style={[s.inp, { backgroundColor: '#F2F4F7', color: '#526E7F' }]} value={kullanici?.meslek || kullanici?.anaBrans || 'Belirtilmemiş'} editable={false} />
              </>
            )}
